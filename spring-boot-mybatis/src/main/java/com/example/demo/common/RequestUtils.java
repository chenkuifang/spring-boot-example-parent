...
public abstract class RequestUtils {
	...
	public static boolean isApplicationJsonHeader(HttpServletRequest request) {
		...
		return contentType != null && StringUtils.replace(contentType.trim(), StringUtils.SPACE, StringUtils.EMPTY)
	 ...
	 * 得到请求地址
	 *
	 * @return 如果是在http://www.google.com域名下请求当前服务器某个api,比如:http://www.goblin.com/api,
	 *         <p>
	 ...
	 * @throws IOException
	 */
	public static String getRequestMessage(HttpServletRequest request, Object userId, String username)
			throws IOException {
		StringBuilder parameters = new StringBuilder();
		parameters.append("\n用户ID : ").append(userId).append("\n用户姓名 : ").append(username);
	...
	private static String getRequestMessage(HttpServletRequest request, StringBuilder parameters) throws IOException {
		parameters.append("\n请求URL : ").append(request.getRequestURI()).append("\n请求URI : ")
				.append(request.getRequestURL()).append("\n请求方式 : ").append(request.getMethod())
				.append(RequestUtils.isAjaxRequest(request) ? "\tajax请求" : "\t同步请求").append("\n请求者IP : ")
